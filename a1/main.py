...
def buildHull(points: list[Point]):
    ...
    # for only two points create a line
    if len(points) == 2:
        # set CW and CCW pointers to correct orientation
    ...
    # for only three points create a triangle
    if len(points) == 3:
        # determine if 3 points for left turn or right turn
        if turn(points[0], points[1], points[2]) == LEFT_TURN:
            # set CW and CCW pointers to correct orientation for left turn triangle
            points[0].ccwPoint = points[1]
            ...
            # set CW and CCW pointers to correct orientation for right turn triangle
            points[0].cwPoint = points[1]
    ...
    # split set of all points into left and right sets of points
    # points array is already ordered in increasing order by x coordinate
    half = len(points) // 2
    # each iteration left array begins contains all points until half value
    left = points[:half]
    # each iteration right array contains all points after and including half value
    right = points[half:]

    # recursively create left ad right hulls
    #   1. recursively call buildHull(left) until left array of points contains
    #   only two points. this happens as half value gets incrementally smaller
    #   by a power of 2 as it is halved each recursion. (supplied number of points
    #   a multiple of 2)
    #       nth left iteration: creates a line between two unused left most points
    #       (in left subsection)
    #   2. call buildHull(right) with same half value
    #       nth left iteration: creates a line between two unused points immediately
    #       right of the previously created two left most unused points (in the left
    #       subsection)

    buildHull(left)
    buildHull(right)

    # Proceed to merging lines to create sub-convex hulls

    # keep track of points stepped over during merging of sub-convex hulls or walk
    # up or walk down
    usedPoints: set[Point] = set()

    # walk up
    # access last point in array of left points
    leftPoint: Point = left[-1]
    # access first point in array of right points
    rightPoint: Point = right[0]

    # ---LEFT OFF HERE---
    # if the left or right point can step up
    while ( turn(leftPoint.ccwPoint, leftPoint, rightPoint) == LEFT_TURN
            or turn(leftPoint, rightPoint, rightPoint.cwPoint) == LEFT_TURN
    ):
        # step up
        if turn( leftPoint.ccwPoint, leftPoint, rightPoint) == LEFT_TURN:
            usedPoints.add(leftPoint)
            assert leftPoint.ccwPoint is not None
            leftPoint = leftPoint.ccwPoint
        else:   # step down
            usedPoints.add(rightPoint)
            assert rightPoint.cwPoint is not None
            rightPoint = rightPoint.cwPoint

    # save reference to points for top segment to prevent modifying the original
    ...
    while ( turn(leftPoint.cwPoint, leftPoint, rightPoint) == RIGHT_TURN
            or turn(leftPoint, rightPoint, rightPoint.ccwPoint) == RIGHT_TURN
    ):
        # step up
        if turn(leftPoint.cwPoint, leftPoint, rightPoint) == RIGHT_TURN:
            usedPoints.add(leftPoint)
            assert leftPoint.cwPoint is not None
    ...
    # saving reference to point points for consistency
    loRight = rightPoint
    loLeft = leftPoint

    # joining top and bottom segments to form the outer hull
    hiLeft.cwPoint = hiRight
    hiRight.ccwPoint = hiLeft
    loLeft.ccwPoint = loRight
    loRight.cwPoint = loLeft

    # remove point on the hull to not accidentally remove cw and ccw
    # references
    usedPoints -= {hiRight, hiLeft, loRight, loLeft}

    # remove cw and ccw references for all used points
