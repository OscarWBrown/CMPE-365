...
def buildTriangles( slice0, slice1 ):
    ...
    for V0 in slice0.verts:
        ...
        for V1 in slice1.verts:
            if minL == 0:
                minL = math.dist(V0.coords, V1.coords)
                i1 = count1
                i0 = count0
                closeV0 = V0
                closeV1 = V1
            elif math.dist(V0.coords, V1.coords) < minL:
                minL = math.dist(V0.coords, V1.coords)
                i1 = count1
                i0 = count0
                closeV0 = V0
    ...
    numV0 = len(slice0.verts)
    tempV = closeV0
    newList0 = []
    tempI0 = i0
    while True:
        newList0.append(slice0.verts[tempI0])
        tempI0 = (tempI0 + 1) % numV0
        if tempI0 == i0:
            break
    newList0.append(slice0.verts[i0])

    numV1 = len(slice1.verts)
    tempV = closeV1
    newList1 = []
    tempI1 = i1
    while True:
        newList1.append(slice1.verts[tempI1])
        tempI1 = (tempI1 + 1) % numV1
        if tempI1 == i1:
            break
    newList1.append(slice1.verts[i1])
    ...
    minArea = [[0 for x in range(numV0 + 1)] for y in range(numV1 + 1)] # CHANGE THIS
    minDir = [[0 for x in range(numV0 + 1)] for y in range(numV1 + 1)] # CHANGE THIS
    ...
    tempV0 = closeV0
    tempV1 = closeV1
    for x in range(1, numV1 + 1):
        for y in range(1, numV0 + 1):
            minAreaLeft = minArea[x][y - 1] + triangleArea(newList0[y - 1].coords, newList0[y].coords, newList1[x].coords)
            minAreaUp = minArea[x - 1][y] + triangleArea(newList1[x - 1].coords, newList0[y].coords, newList1[x].coords)
            if minAreaLeft < minAreaUp:
                minArea[x][y] = minAreaLeft
                minDir[x][y] = Dir.PREV_COL
            else:
                minArea[x][y] = minAreaUp
    ...
    print(minArea[a][b])
    while True:
        if minDir[a][b] == Dir.PREV_ROW:
            newT = Triangle([newList1[a], newList0[b], newList1[a - 1]])
            triangles.append(newT)
            a -= 1
        else:
            newT = Triangle([newList1[a], newList0[b], newList0[b - 1]])
            triangles.append(newT)
            b -= 1
        if a == 0 and b == 0:
            break
