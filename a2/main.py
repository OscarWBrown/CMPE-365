...
def buildTristrips(triangles):
    count = 0
    # search for triangle with least amount of neighbours
    # set first triangle in array of triangles to current triangle
    currentTri = triangles[0]
    # set counter for number of adjacent triangles
    minAdjCount = len(currentTri.adjTris)
    # iterate through array of triangles beginning after first triangle
    for tri in triangles[1:]:
        adjCount = len(tri.adjTris)
        # update minimum adjacent count for each triangle checked
        # every time a new triangle with a new minimum adjacent triangles is found
        # set this triangle to the new temporary starting triangle for new strip
        if adjCount < minAdjCount:
            minAdjCount = adjCount
            currentTri = tri

    usedTris = set()
    # returns an array list of all unused adjacent neighbours for a certain triangle
    def unusedNeighbours(triangle):
        unusedN = []
        for tri in triangle.adjTris:
            if tri not in usedTris:
                unusedN.append(tri)
        return unusedN
    # loop runs indefinitely
    while True:
        # begin new strip with triangle previously identified as starting triangle
        usedTris.add(currentTri)
        neighbours = unusedNeighbours(currentTri)
        # strip ends when current triangle has no unused adjacent neighbours
        if len(neighbours) == 0:
            count = count + 1
            # algorithm ends when the amount of used triangles is equivalent to amount of total triangles
            if len(usedTris) == len(triangles):
                break
            # search for triangle with least amount of unused adjacent neighbours
            # set this triangle as the first triangle in the new strip
            minAdjCount = 4
            for tri in triangles:
                if tri in usedTris:
                    continue
                adjCount = len(unusedNeighbours(tri))
                if adjCount < minAdjCount:
                    minAdjCount = adjCount
                    currentTri = tri
            continue
        # if triangle with one unused adjacent triangle exists
        # add this triangle to the list through adjusting next and previous pointers
        if len(neighbours) == 1:
            nextTri = neighbours[0]
            currentTri.nextTri = nextTri
            nextTri.prevTri = currentTri
            currentTri = nextTri
            continue
        # proceed with shortest route
        # check if length of neighbour a is zero or less than length of neighbour b
        if len(unusedNeighbours(neighbours[0])) == 0 or len(unusedNeighbours(neighbours[0])) < len(unusedNeighbours(neighbours[1])):
            nextTri = neighbours[0]
            currentTri.nextTri = nextTri
            nextTri.prevTri = currentTri
            currentTri = nextTri
            continue
        # check if length of neighbour b is zero or less than length of neighbour a
        elif len(unusedNeighbours(neighbours[1])) == 0 or len(unusedNeighbours(neighbours[1])) < len(unusedNeighbours(neighbours[0])):
            nextTri = neighbours[1]
            currentTri.nextTri = nextTri
            nextTri.prevTri = currentTri
            currentTri = nextTri
        # check to see if cases are equal
        else:
            minAdjCount = 4
            nextTriangle = neighbours[0]
            # search neighbour[0]'s neighbours
            for tri in unusedNeighbours(neighbours[0]):
                adjCount = len(unusedNeighbours(tri))
                if adjCount < minAdjCount:
                    minAdjCount = adjCount
                    nextTriangle = neighbours[0]
            # search neighbour[1]'s neighbours
            for tri in unusedNeighbours(neighbours[1]):
                adjCount = len(unusedNeighbours(tri))
                if adjCount < minAdjCount:
                    minAdjCount = adjCount
                    nextTriangle = neighbours[1]
            # readjust pointers as necessary
            currentTri.nextTri = nextTriangle
            nextTriangle.prevTri = currentTri
            currentTri = nextTriangle
